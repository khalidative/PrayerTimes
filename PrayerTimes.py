...
selected_month = st.select_slider('Select Month:', options=months, value=months[date.today().month - 1])
...
if st.button('Calculate Prayer Times'):
    ...
    # Convert prayer times to DataFrame for display
    ...
    # CSV Export
    csv = df_prayer_times.to_csv(index=False)
    st.download_button(
        label="Download Prayer Times as CSV",
        data=csv,
