...
class PrayTimes():
	...
	def asrFactor(self, asrParam):
		...
		return methods.get(asrParam, self.eval(asrParam))
...
def calculate_monthly_prayer_times(year, month, latitude, longitude, timezone, dst, calc_method, asr_method):
    PT = PrayTimes(calc_method)
    PT.adjust({'asr': asr_method})  # Adjusting the Asr calculation method
    num_days = calendar.monthrange(year, month)[1]
    prayer_times = []
    for day in range(1, num_days + 1):
        times = PT.getTimes((year, month, day), (latitude, longitude, 0), timezone, dst)
        date_str = datetime(year, month, day).strftime('%Y-%m-%d')
        formatted_times = {'Date': date_str}
        formatted_times.update({time_name: times[time_name.lower()] for time_name in PT.timeNames.values()})
...
def calculate_monthly_prayer_times(year, month, latitude, longitude, timezone, dst, calc_method):
    ...
    for day in range(1, num_days + 1):
        ...
        for asr_method in ['Standard', 'Hanafi']:
            ...
            # Append method name to prayer times to differentiate
            for time_name in ['Fajr', 'Sunrise', 'Dhuhr', 'Maghrib', 'Isha']:
                key_name = f"{time_name} ({asr_method})"
                day_times[key_name] = times[time_name.lower()]
            
            # Asr is calculated differently, so it's not included in the loop
            day_times[f"Asr ({asr_method})"] = times['asr']

        prayer_times.append(day_times)
...
def calculate_monthly_prayer_times(year, month, latitude, longitude, timezone, dst, calc_method):
    ...
    prayer_times = []
    for day in range(1, num_days + 1):
        times_standard = PT.getTimes((year, month, day), (latitude, longitude, 0), timezone, dst, 'Standard')
        times_hanafi = PT.getTimes((year, month, day), (latitude, longitude, 0), timezone, dst, 'Hanafi')
        date_str = datetime(year, month, day).strftime('%Y-%m-%d')
        formatted_times = {'Date': date_str}
        for time_name in ['Fajr', 'Sunrise', 'Dhuhr', 'Sunset', 'Maghrib', 'Isha']:
            formatted_times[time_name] = times_standard[time_name.lower()]
        formatted_times['Asr (Standard)'] = times_standard['asr']
        formatted_times['Asr (Hanafi)'] = times_hanafi['asr']
        prayer_times.append(formatted_times)
    return prayer_times
...
def calculate_monthly_prayer_times(year, month, latitude, longitude, timezone, dst, calc_method):
    ...
    for day in range(1, num_days + 1):
        date_str = datetime(year, month, day).strftime('%Y-%m-%d')
        # Calculate times using standard method for Asr
        PT.adjust({'asr': 'Standard'})
        times_standard = PT.getTimes((year, month, day), (latitude, longitude, 0), timezone, dst)
        # Calculate times using Hanafi method for Asr
        PT.adjust({'asr': 'Hanafi'})
        times_hanafi = PT.getTimes((year, month, day), (latitude, longitude, 0), timezone, dst)

        # Merge results, selecting Asr from both calculations
        formatted_times = {'Date': date_str,
                           'Imsak': times_standard['imsak'],
                           'Fajr': times_standard['fajr'],
                           'Sunrise': times_standard['sunrise'],
                           'Dhuhr': times_standard['dhuhr'],
                           'Asr (Standard)': times_standard['asr'],
                           'Asr (Hanafi)': times_hanafi['asr'],
                           'Sunset': times_standard['sunset'],
                           'Maghrib': times_standard['maghrib'],
                           'Isha': times_standard['isha'],
                           'Midnight': times_standard['midnight']}
        prayer_times.append(formatted_times)
    return prayer_times

# Function to get prayer times for the current day for all calculation methods
def get_prayer_times_for_all_methods(date, latitude, longitude, timezone, dst):
    methods = ['MWL', 'ISNA', 'Egypt', 'Makkah', 'Karachi', 'Tehran', 'Jafari']
    prayer_times = []

    for method in methods:
        PT = PrayTimes()
        PT.setMethod(method)  # Correctly set the calculation method
        PT.adjust({'asr': 'Hanafi'})  # Adjust Asr calculation to Hanafi
        times = PT.getTimes(date, (latitude, longitude, 0), timezone, dst)
        times['method'] = method  # Keep track of the method used
        prayer_times.append(times)
    
    return prayer_times

# Streamlit UI
st.title('PrayTimes Class Functionality Tester')

# User inputs
date_input = st.date_input("Select Date:", datetime.today())
latitude = st.number_input("Latitude:", value=11.495351, format='%f')
longitude = st.number_input("Longitude:", value=79.759439, format='%f')
timezone = st.number_input("Timezone Offset from UTC:", value=5.5, format='%f')
dst = st.checkbox("Daylight Saving Time")

if st.button("Calculate Prayer Times"):
    # Convert selected date to the format expected by PrayTimes
    selected_date = (date_input.year, date_input.month, date_input.day)
    # Get prayer times
    prayer_times = get_prayer_times_for_all_methods(selected_date, latitude, longitude, timezone, dst)
    # Convert to DataFrame for display
    df_prayer_times = pd.DataFrame(prayer_times)
    # Set method name as the DataFrame index
    df_prayer_times.set_index('method', inplace=True)
